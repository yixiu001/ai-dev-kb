...
用途：帮助用户按步骤完成真实项目。可以包含故事、比喻、案例、流程图、工作表和可复制提示词。
...
当用户属于以下情况之一时，必须读取 `01-ai-collaboration/NOVICE_GUIDANCE_PROTOCOL.md`：
...
不要默认读取全部用户文章。只有在需要引导用户执行某个阶段、提供教学示例或确认用户操作方式时，才读取对应的 `00-user-guide/` 文章。
...
| 如何引导完全没有开发经验的用户 | `01-ai-collaboration/NOVICE_GUIDANCE_PROTOCOL.md` |
...
当用户不理解时，不得简单重复原解释或继续增加术语。必须缩小为一个可以执行和观察的动作。
